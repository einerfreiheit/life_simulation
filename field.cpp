...
void field::step(){
	...
	for (int i=0;i<worm_map.size();i++){
		...
		logic.move(logic.IsHungry(map[y][x].life_res),y,x,mapHeight,mapWidth);
		x=logic.next_x;
		y=logic.next_y;
...
void field::getVis(){
	...
	for (int i=0;i<worm_map_size;i++){
		vis.at<cv::Vec3b>(cv::Point(worm_map[i].worm_y,worm_map[i].worm_x))[2]=255;
		vis.at<cv::Vec3b>(cv::Point(worm_map[i].worm_y,worm_map[i].worm_x))[0]=0;
		vis.at<cv::Vec3b>(cv::Point(worm_map[i].worm_y,worm_map[i].worm_x))[1]=0;
		if (worm_map[i].worm_hungry==true){
			vis.at<cv::Vec3b>(cv::Point(worm_map[i].worm_y,worm_map[i].worm_x))[2]=0;
		}
