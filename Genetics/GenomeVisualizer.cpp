...
void GenomeVisualizer::drawGene(const Gene &gene, cv::Mat &vis, int posX, int posY) {
	...
	cv::rectangle(	vis,
					cv::Rect(posX + geneWidth - geneWidth / 6, posY, geneWidth / 6, geneHeight),
					getAllelColor(gene.allel2),
					-1);
	cv::putText(vis,
				std::to_string(gene.type),
				cv::Point(posX + geneWidth / 6, posY + geneHeight - 2),
				CV_FONT_NORMAL,
				0.6,
				cv::Scalar(255, 0, 0),
				2);
